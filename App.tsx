...
export default function App() {
  ...
  const interpretation = useMemo(
    () => analysis ? interpretSoil(analysis) : [],
    [analysis]
  );
  ...
  async function handlePdf() {
    ...
    try {
      ...
      await exportReportToPdf(
        'report-area',
        `laudo-solo-${profile.name.toLowerCase().replaceAll(' ', '-')}.pdf`
      );
    } catch (err) {
    ...
    <main className="app">
      <header className="hero">
        <div>
          ...
          <p>
            Faça upload da análise, selecione a cultura, gere a interpretação visual
            e exporte o laudo técnico em PDF.
          </p>
        </div>
      ...
      <section className="panel">
        ...
        <label className="upload-box">
          ...
          />
          <span>
            {analysis?.fileName || 'Clique para carregar a planilha de análise de solo'}
          </span>
        </label>
      ...
      <section className="panel controls">
        ...
        <div className="form-grid">
          <label>
            ...
            <select
              value={culture}
              onChange={(e) => onCultureChange(e.target.value as CultureKey)}
            >
              {CULTURES.map((c) => (
          ...
          <label>
            ...
            <select
              value={limeMethod}
              onChange={(e) => setLimeMethod(e.target.value as LimeMethod)}
            >
          ...
          <label>
            ...
            <select
              value={limeSource}
              onChange={(e) => setLimeSource(e.target.value as LimeSource)}
            >
          ...
          <label>
            ...
            <input
              type="number"
              step="0.1"
              value={targetCaAbsolute}
              onChange={(e) => setTargetCaAbsolute(Number(e.target.value))}
            />
          </label>
          ...
          <label>
            ...
            <input
              type="number"
              step="0.1"
              value={poultryNPercent}
              onChange={(e) => setPoultryNPercent(Number(e.target.value))}
            />
          </label>
          ...
          <label>
            ...
            <input
              type="number"
              value={poultryEfficiency}
              onChange={(e) => setPoultryEfficiency(Number(e.target.value))}
            />
          </label>
        </div>
        ...
        <div className="toggle-row">
          {RECOMMENDATIONS.map((key) => (
            <button
              key={key}
              className={selectedRecommendations.includes(key) ? 'toggle active' : 'toggle'}
      ...
      <section id="report-area" className="report">
        <div className="report-header">
          <div>
            ...
            <p>
              Cultura selecionada: <strong>{profile.name}</strong>
            </p>
          </div>
        ...
        <section className="panel white">
          ...
          {analysis ? (
            <InterpretationTable items={interpretation} />
          ) : (
            <div className="empty">
              Carregue uma análise para visualizar a classificação.
            </div>
          )}
        </section>
        ...
        <section className="panel white">
          ...
          <ul className="notes">
            ...
            <li>
              Quando a análise não traz argila, silte e areia, o sistema mantém a
              recomendação sem usar textura.
            </li>
            <li>
              Valide os parâmetros com um responsável técnico antes de uso comercial.
            </li>
          </ul>
        </section>
      </section>
      ...
      <div className="action-bar">
        <button
          ...
          onClick={handlePdf}
        >
          {isExporting ? 'Gerando PDF...' : 'Gerar laudo em PDF'}
        </button>
      </div>
    </main>
  );
